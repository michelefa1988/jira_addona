...
function createPDFDocument() {
    ...
    var stream = fs.createWriteStream(process.env.pdf_filename);
    doc.pipe(stream);
    ...
    writetoPDF: function(AddedToSprint) {
            ...
            //     //map used for shortening project names
                ...
                page.forEach(function(ticket) {
                    ...
                    //colour the ticket according to type of ticket
                    ...
                    doc.font('./Assets/Fonts/Times_New_Roman_Bold.ttf')
                        .fontSize(50)
                        .fillColor('black')
                        .text(page[ticketNo][process.env.printed_tickets_center_bold].replace(re, function(matched) {
                    ...
                    doc.text(polishString(page[ticketNo][process.env.printed_tickets_small_text]));

                    //display story points
                    // doc.font('./assets/fonts/Times_New_Roman_Normal.ttf')
                    //     .fontSize(30)
                    //     .fillColor('black')
                    //     .text(page[ticketNo][process.env.printed_tickets_topRight], x + 200, (y * 260) + 10);


                    //display Asignee first name
                    var FullName = page[ticketNo][process.env.printed_tickets_topLeft].split(" ");
                    doc.font('./Assets/Fonts/Times_New_Roman_Normal.ttf')
                        .fontSize(20)
                        .fillColor('black')
